    ...
    public class PieceDrop : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            RaycastHit2D centerHit =
                Physics2D.Raycast(transform.position + new Vector3(0, -sprite.bounds.size.y / 2 - 1f), Vector2.down,
                    Mathf.Infinity);
            ...
            if (centerDistance <= rightDistance && centerDistance <= leftDistance)
            {
                ...
                projection.transform.position = new Vector3(transform.position.x,
                    centerHit.point.y + projection.GetComponent<SpriteRenderer>().bounds.size.y / 2);
            ...
            else
            {
                ...
                {
                    Debug.DrawRay(transform.position + new Vector3(-sprite.bounds.size.x / 2 + RAYCAST_VARIATION, 0),
                        Vector2.down * 15f, Color.red);
                    //Colisiona izquierda
                    targetPos = leftHit.point + new Vector2(sprite.bounds.size.x / 2, sprite.bounds.size.y / 2);
                    projection.transform.position = new Vector3(transform.position.x,
                        leftHit.point.y + projection.GetComponent<SpriteRenderer>().bounds.size.y / 2);
                ...
                else if (leftDistance > rightDistance)
                {
                    ...
                    projection.transform.position = new Vector3(transform.position.x,
                        rightHit.point.y + projection.GetComponent<SpriteRenderer>().bounds.size.y / 2);
        ...
        {
            RaycastHit2D centerHit =
                Physics2D.Raycast(transform.position + new Vector3(0, -sprite.bounds.size.y / 2 - 0.5f), Vector2.down,
                    0.5f);
            RaycastHit2D rightHit = Physics2D.Raycast(transform.position + new Vector3(sprite.bounds.size.x / 2, 0),
                Vector2.right, 0.5f);
            RaycastHit2D leftHit = Physics2D.Raycast(transform.position + new Vector3(-sprite.bounds.size.x / 2, 0),
                Vector2.left, 0.5f);
